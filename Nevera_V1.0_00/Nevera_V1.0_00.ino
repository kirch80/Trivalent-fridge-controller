...
const StructMenu Menu PROGMEM = {                                                 // Inicializa struct Menu con textos
  ...
  { 3, 4, 3, 7, 3},                                                               // Inicializa numero de lineas por menu/submenu -1
  {
...
void ScreenMainConfig(int LongScreen, int ShortScreen) {                          // Subrutina configuracion pantalla menu
  ...
  if (!AlarmActiveFlag && !DoorOpen) {                                            // Si no tengo ni alarmas ni puerta abierta
    ...
    auxMap = dsTemp[AuxMenu];                                                     // Copio temperatura sensor a auxMap
    CheckMinMax(&auxMap, AuxSens.MinValue, AuxSens.MaxValue);                     // Compruebo limites
  ...
  if (!dsConfig[AuxMenu]) {                                                       // Si no tengo error de lectura sensor de temperatura
    DrawString (C, &Text.TextItems[4], 2, 0, 12, 14, Puls_1s);                    // Escribe texto "Error!" 
  } else {                                                                        // Si tengo error de lectura sensor de temperatura
    if (dsTemp[AuxMenu] <= 999) {
      DrawInt (C, dsTemp[AuxMenu], &Units.UnitsItems[0], 3, 0, 10, true, false);  // Escribe temperatura con un decimal
    } else {
      DrawInt (C, (dsTemp[AuxMenu] / 10), &Units.UnitsItems[0], 3, 0, 10, false, false); // Escribe temperatura sin decimales      
    }
  }
  CheckMinMaxToEnc(&AuxMenu, 0, 3);                                               // Cargo y configuro limites encoder
...
void SerialPlotter() {                                                            // Subrutina pinta grafica
  ...
  if (Puls_1m_FlUp) {
    Serial.print(dsTemp[0] / 10.0);                                               // Pinta temperatura nevera cada segundo
...
void GetDS18b20() {                                                               // Subrutina inicializacion y lectura sensores DS18b20
  ...
  ds0.begin((int)&dsConfig[0], &dsTemp[0], &dsCont[0]);                           // Inicializacion y lectura sensor 0
  ds1.begin((int)&dsConfig[1], &dsTemp[1], &dsCont[1]);                           // Inicializacion y lectura sensor 1 
  ds2.begin((int)&dsConfig[2], &dsTemp[2], &dsCont[2]);                           // Inicializacion y lectura sensor 2 
  ds3.begin((int)&dsConfig[3], &dsTemp[3], &dsCont[3]);                           // Inicializacion y lectura sensor 3 
}
