...
//  Conversor de imagen a hex             http://javl.github.io/image2cpp/
//
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
#define ACDC_CONST      66                                                        // Define sensivilidad en mV/A sensor ACS712 30A
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
const unsigned char TERM[]    PROGMEM = {
  ...
  0xd0, 0x80, 0x10, 0x80, 0xd0, 0x80, 0x10, 0x80, 0x50, 0x80, 0x10, 0x80, 0xd0, 0x80, 0x10, 0x80, 
...
const StructBitmapOffOn FIRE PROGMEM = {                                          // Inicializa struct FIRE para animacion
  {                                                                     
    {
     0x00, 0x00, 0x01, 0x80, 0x01, 0xc0, 0x03, 0xc0, 0x03, 0xe8, 0x07, 0xfc, 0x1f, 0xfc, 0x3f, 0x7e,
     0x3f, 0x3e, 0x7f, 0x1e, 0x7c, 0x1e, 0x78, 0x1e, 0x78, 0x1c, 0x30, 0x1c, 0x10, 0x18, 0x00, 0x00
    },
    {
     0x00, 0x00, 0x01, 0x80, 0x03, 0x80, 0x03, 0xc0, 0x17, 0xc0, 0x3f, 0xe0, 0x3f, 0xf8, 0x7e, 0xfc,
     0x7c, 0xfc, 0x78, 0xfe, 0x78, 0x3e, 0x78, 0x1e, 0x38, 0x1e, 0x38, 0x0c, 0x18, 0x08, 0x00, 0x00
    }
  }    
};
...
bool          DoorOpen = 0;                                                       // Variable puerta abierta
...
struct StructTextItems {                                                          // Struct para textos generales
  StructItem12 TextItems[21];                                                     // Tamaño de X textos
};
...
const StructMenu Menu PROGMEM = {                                                 // Inicializa struct Menu con textos
  {"A1.04v"},                                                                     // Inicializa texto version de software 
  ...
  { 3, 3 + CONGELADOR, (NOFROST * 3), 2 + PUERTA + (AMPSENSOR * 4), 3},           // Inicializa numero de lineas por menu/submenu -1
  {
    {                                                                             // Inicializa submenu "TEMPERATURAS"
      {"NEVERA"      ,  3,  100,   30, B00001111, &Mem.Data.TempSp[0]},           // SP nevera 
    ...
    {                                                                             // Inicializa submenu "VENTILADORES"
      ...
      #if NOFROST                                                                 // Si tengo ventilador nevera instalado
...
void ScreenMainConfig(int LongScreen, int ShortScreen) {                          // Subrutina configuracion pantalla principal
  ...
  if (!AlarmActiveFlag && !DoorOpen) {                                            // Si no tengo ni alarmas ni puerta abierta
    ...
    lcd.fillRect(75, 25, 4, map(AuxMap, (*AuxSens.ParamPointer - 100), (*AuxSens.ParamPointer + 100), 0, -22), BLACK);// Relleno termometro dependiendo valores +-10ºC                
  ...
  if (MenuIndex == 0) {                                                           // Si MenuIdex = 0, muestra pantalla normal
    ...
    #else
      if (VoltsBat < 60) {                                                        // Si voltaje en inferior a 6.0v
        DrawString (C, &Text.TextItems[19], 1, 0, 52, 32, false);                 // Escribe texto "USB" 
      } else {                                                                    // Si voltaje en superior a 6.0v
        DrawString (C, &Text.TextItems[18], 1, 0, 50, 32, false);                 // Escribe texto "V.BAT" 
      }
    #endif
    if (VoltsBat < 60) {                                                          // Si voltaje leido es menos de 6.0v
      DrawInt (C, (CPUmVolt / 100), &Units.UnitsItems[1], 1, 1, 40, true, (VoltsBatAlarmFlag && Puls_05s)); // Dibuja voltaje USB     
    } else {                                                                      // Si voltaje leido es mayor de 6.0v
    ...
    if (Mem.Data.Mode == 0) {                                                     // Si modo OFF
      ...
      #else
        lcd.drawFastHLine(33, 40, 16, BLACK);                                     // Escribe linea en lugar de ventilador nevera
      #endif
      lcd.drawBitmap( 68, 32,  FAN.BitmapOffOn[(FanAmbAux > 0 && Puls_05s)].bitmap32, 16, 16, BLACK); // Pinta ventilador ambiente       
    }
  }
...
void FactoryReset() {                                                             // Subrutina valores de fabrica
  ...
  if (digitalRead(EBTN) == LOW) {
    ...
    Mem.Data.TempSp[2] =  385;                                                    // Valor de fabrica consigna ambiente 
    Mem.Data.TempSp[3] = 1050;                                                    // Valor de fabrica consigna hervidor
    Mem.Data.TempSp[4] =  850;                                                    // valor de fabrica consigna standby hervidor

    Mem.Data.RpmVentAmb = 90;                                                     // Valor de fabrica rpm ventilador ambiente   
    Mem.Data.RpmVentNev = 90;                                                     // Valor de fabrica rpm ventilador nevera
    Mem.Data.RpmVentNevOn = 5;                                                    // Valor de fabrica minutos On ventilador nevera  
    Mem.Data.RpmVentNevOff = 10;                                                  // Valor de fabrica minutos Off ventilador nevera  
    ...
    Mem.Data.AmpsAlarmLowAC = 01;                                                 // Variable alarma consumo bajo AC
        
    Mem.Data.Brillo = 40;                                                         // Valor de fabrica brillo
    Mem.Data.Contrast = 75;                                                       // Valor de fabrica contraste
    Mem.Data.BackLightTime = 30;                                                  // Valor de fabrica retroiluminacion
    Mem.Data.BackMenuTime = 20;                                                   // Valor de fabrica retroceso pantalla inactiva
...
//
  ...
  if (Mem.Data.Mode > 0) {                                                        // Si ciclo en marcha
  ...
  if ((Mem.Data.Mode > 0) && (GasLowAlarmFlag || VoltsBatAlarmFlag || AmpsAlarmDCFlag || AmpsAlarmACFlag || !dsConfig[0] || !dsConfig[1] || !dsConfig[2] || !dsConfig[3])) { // Si tengo ciclo y alarmas 
